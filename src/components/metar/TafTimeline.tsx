        ...
        {segments.map((segment) => (
          <article key={segment.id} className={`rounded-2xl border p-4 ${riskClass[segment.risk]}`}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="rounded-full border border-current/30 px-2 py-1 text-xs font-black uppercase">{segment.risk}</span>
            </div>
            ...
            <details className="mt-3">
              <summary className="cursor-pointer text-xs font-bold uppercase tracking-wide opacity-80">Raw TAF segment</summary>

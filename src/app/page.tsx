...
const WOW_POINTS = [
  { key: "mission", color: "cyan", pl: "Misje na żywo z decyzją GO / CAUTION / NO-GO", en: "Live Mission Mode with GO / CAUTION / NO-GO calls" },
      ...
      <main className="flex w-full flex-col gap-10 py-4">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              ...
              <p className="mt-6 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
                {pl ? "METAR Quest łączy quiz, pogodę na żywo, briefing kokpitowy i misje GO/NO-GO, aby nauka przypominała prawdziwe przygotowanie do lotu." : "METAR Quest blends quiz training, live weather, cockpit briefing and GO/NO-GO missions so learning feels like a real preflight briefing."}
              </p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/missions" className="inline-flex items-center justify-center rounded-xl border border-cyan-300/60 bg-cyan-300/10 px-6 py-3 text-sm font-black text-cyan-100 transition hover:bg-cyan-300/20">
                  {pl ? "Misja dnia" : "Daily Live Mission"}
                </Link>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-slate-950/60 p-5 text-white backdrop-blur">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3 text-center text-xs">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="text-slate-400">Wiatr</p><p className="mt-1 font-mono font-bold">28014KT</p></div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="text-slate-400">Widzialność</p><p className="mt-1 font-mono font-bold">10SM</p></div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="text-slate-400">Chmury</p><p className="mt-1 font-mono font-bold">FEW015</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {WOW_POINTS.map((point) => (
            <article key={point.key} className="animate-in rounded-2xl border border-sky-300/20 bg-[var(--surface)]/85 p-5 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-cyan-300/50 hover:shadow-cyan-500/10">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { href: "/time-attack", title: t("timeAttack"), desc: pl ? "Trening na czas z seriami, rangami i rekordami." : "Arcade training with combo, ranks and records." },
            { href: "/real-weather", title: t("realWeather"), desc: pl ? "METAR/TAF na żywo z briefingiem pilota." : "Live METAR/TAF with pilot briefing." },
            { href: "/briefing-lab", title: pl ? "Laboratorium briefingu" : "Briefing Lab", desc: pl ? "Trasa, wiatr boczny, odtworzenie pogody i omówienie instruktora." : "Route, crosswind, weather replay and instructor debrief." },
            { href: "/progress", title: t("progress"), desc: pl ? "Ranga pilota, odznaki i plan treningu." : "Pilot rank, badges and training plan." },
          ].map((item) => (
            <Link key={item.href} href={item.href} className="animate-in rounded-2xl border border-slate-500/40 bg-[var(--surface)]/85 p-6 transition-all duration-300 hover:-translate-y-1 hover:border-sky-400/50 hover:shadow-xl hover:shadow-sky-500/10 active:scale-[0.99]">

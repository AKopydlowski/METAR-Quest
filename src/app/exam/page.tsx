  ...
  return (
    <div className="w-full space-y-6">
      <section className="rounded-[2rem] border border-violet-300/20 bg-slate-950/80 p-6 text-white shadow-2xl shadow-violet-950/30">
        ...
        <p className="mt-3 max-w-2xl text-sm text-slate-300">{pl ? "25 pytań, brak podpowiedzi, próg zaliczenia 80%. Wynik zapisuje się w progresie i rankingu." : "25 questions, no hints, 80% pass mark. The result is saved to progress and leaderboard."}</p>
      </section>
      ...
      <section className="rounded-3xl border border-slate-500/30 bg-[var(--surface)]/90 p-6 shadow-xl">
        {finished ? (
          <div className="space-y-4">
            ...
            <p>{score}/{questions.length} • próg {PASS_MARK}%</p>
          ...
          <>
            <div className="flex items-center justify-between gap-3"><p className="text-sm font-bold">{current + 1}/{questions.length}</p><p className="text-sm">Wynik: {score}</p></div>
            ...
            <article className="mt-5 rounded-2xl border border-slate-700 bg-slate-950/40 p-4"><p className="text-xs uppercase tracking-wide text-slate-400">{q.skillTag} • {q.difficulty === "easy" ? "łatwe" : q.difficulty === "medium" ? "średnie" : "trudne"}</p><p className="mt-3 text-lg font-semibold">{q.prompt}</p><p className="mt-3 rounded-xl bg-slate-900 p-3 font-mono text-sm text-cyan-100">{q.metarRaw}</p></article>
            ...
            {selectedId && <button onClick={next} className="mt-4 rounded-xl bg-violet-400 px-4 py-2 font-black text-slate-950">{current >= questions.length - 1 ? "Zakończ" : "Następne"}</button>}
          </>
        )}
      </section>
    </div>
